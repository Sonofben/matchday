...
async function cleanup() {
    ...
    `, ["Saturday's Baller Cup 2026"]);
        for (const comp of demoComps) {
            ...
            const { rows: matches } = await client.query('SELECT id FROM matches WHERE competition_id = $1', [comp.id]);
            ...
            const { rows: rounds } = await client.query('SELECT id FROM bracket_rounds WHERE competition_id = $1', [comp.id]);
