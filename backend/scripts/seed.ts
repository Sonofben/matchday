...
async function seed() {
  ...
  for (const pl of samplePlayers) {
    const existing = await db.query(
      `SELECT id FROM players WHERE first_name=$1 AND last_name=$2`, [pl.first, pl.last]
    )
    let playerId: string
    if (existing.rows[0]) {
  ...
  const matchCheck = await db.query(
    `SELECT id FROM matches WHERE home_team_id=$1 AND away_team_id=$2`, [teamIds[0], teamIds[1]]
  )
